...
public class ListTournament {
    ...
    private final ObservableList<Tournament> lsTournaments = FXCollections.observableArrayList();
    ...
    public ObservableList<Player> getPlayers(int numTournament) {
        return lsTournaments.get(numTournament).getPlayers();
    }

    //retourne tous les adversaires d'un joueur
    public ObservableList<Player> getOpponents(int numTournament, Player p) {
    ...
    public ObservableList<Match> getMatchs(int numTournament) {
    ...
    private void initData() {
        ...
        for (int i = 0; i < 10; i += 2) {
            Match m = new Match(new Player(nom[i]), new Player(nom[i + 1]), Match.Result.MATCH_NULL);
        ...
        for (int i = 8; i < 10; i += 2) {
            Match m = new Match(new Player(nom[i]), new Player(nom[i + 1]), Match.Result.MATCH_NULL);
            lsTournaments.get(1).addMatch(m);
        }
